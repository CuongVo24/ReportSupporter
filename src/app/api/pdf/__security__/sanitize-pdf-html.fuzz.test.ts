...
describe("stripKnownPdfHazardsBestEffort — cases it correctly strips", () => {
  it("removes a plain <script> block", () => {
    const out = stripKnownPdfHazardsBestEffort('<p>hi</p><script>document.body.textContent="SCRIPT_RAN"</script>');
    expect(out).not.toContain("<script");
    expect(out).not.toContain("SCRIPT_RAN");
  });

  it("removes quoted inline event handlers regardless of case", () => {
    expect(stripKnownPdfHazardsBestEffort(`<img src="a.png" onerror="alert(1)">`)).not.toContain("onerror");
    expect(stripKnownPdfHazardsBestEffort(`<img src="a.png" ONERROR="alert(1)">`)).not.toContain("ONERROR");
    expect(stripKnownPdfHazardsBestEffort(`<body onload='alert(1)'>`)).not.toContain("onload");
  });

  it("blanks absolute http(s) src/href on img/link", () => {
    expect(stripKnownPdfHazardsBestEffort(`<img src="https://evil.example/tracker.png">`)).toContain(`src=""`);
    expect(stripKnownPdfHazardsBestEffort(`<link href="http://evil.example/x.css">`)).toContain(`href=""`);
  });
  ...
  it("bounded fuzz: seeded combinations of script/event-handler/link noise never leave a runnable script tag", () => {
    ...
    for (let i = 0; i < 40; i++) {
      ...
      const out = stripKnownPdfHazardsBestEffort(input);
      expect(out.toLowerCase()).not.toContain("<script");
    }
  });
});

describe("stripKnownPdfHazardsBestEffort — known bypasses (documented residual risk, not a passing security claim)", () => {
