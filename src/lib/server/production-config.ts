// W24-H: Production deployment config contract — server-only.
//
 ...
 *  - pdfEnabled       → PDF_RENDERER_URL/TOKEN pairing is required (defaults to
 *                       "URL is present", so a preview deploy without PDF is exempt).
 */
export function validateProductionConfig(
  ...
  if (!PROXY_MODES.includes(proxyMode as (typeof PROXY_MODES)[number])) {
    ...
    // Without a trusted client IP, every request collapses to the "direct"
    // bucket → the per-user quota becomes one shared quota for the whole beta.
    problems.push({
  ...
  // --- PDF renderer pairing ---
  if (pdfEnabled) {
    if (pdfUrl && !/^https?:\/\//u.test(pdfUrl)) {
      problems.push({
        code: "pdf_url_format",
    ...
    if (!pdfToken) {
      problems.push({
        ...
        message: "Bật PDF renderer thì PDF_RENDERER_TOKEN không được rỗng (renderer sẽ mở cho mọi request).",
      });
...
export function rendererTokenIsInsecure(env: ConfigEnv): ConfigProblem | null {
  ...
  if (!token) {
    return {
      code: "pdf_token_missing",
      variable: "PDF_RENDERER_TOKEN",
      message: "Renderer production không được chạy với PDF_RENDERER_TOKEN rỗng (mở cho mọi request).",
    };
  }
