...
| **Heading Numbering Parity** | HTML / PDF / DOCX | **PASS** | Heading numbering (e.g. `1`, `1.1`, `1.1.1`) is compiled sequentially in the shared pipeline and directly mapped to all three targets. No re-numbering occurs, guaranteeing numbering parity (cùng một nguồn số, map trực tiếp). |
...
| **Math & Equation Fallbacks** | HTML / PDF / DOCX | **PASS (fallback)** | Inline and display math blocks compile to styled LaTeX expressions in HTML/PDF (via client KaTeX). In Word/DOCX, they fallback to structured bracketed monospaced strings to prevent crashes (no OMML rendering). |
| **Mermaid Diagram Fallback** | HTML / PDF / DOCX | **PASS (fallback)** | Renders as client-rendered diagram in HTML. In PDF/DOCX, fallbacks to listing raw Mermaid code block if not pre-rendered. |
| **Browser Print (PDF)** | PDF | **PASS (fallback)** | Browser window print surface writes HTML content cleanly and opens the standard printer dialog. Page numbers, headers, and footers are browser-managed. |
...
* **PDF**: [report.pdf](file:///e:/ReportSupporter/Design/Reports/Month1/W4/samples/report.pdf) (Note: Generated using browser's print dialog via "Save as PDF" from `report.html` print styling, not from native application-level binary compilation).
