...
## 3. Checklist
- [x] `docx` installed exact-pinned; version recorded (v9.7.1).
- [x] `mdast-to-docx.ts` maps heading/paragraph/table/code/image/math; numbering from `FormattedReport` (no re-number).
- [x] Depth > 6 → level 6 + full number in text; orphan `asset:` → placeholder (no throw).
- [x] `export-docx.ts` stub replaced — `exportDocx` **sync** builds `Document` (+ `FormatPreset` styles + cover + TOC), returns `DocxBuildResult` (no Promise); `packDocx(doc)` is the only async (`Packer.toBlob`).
- [x] No Pandoc/LibreOffice/`@react-pdf` import anywhere.
- [x] Tests cover mapping + blob output + orphan-image placeholder + math fallback.
- [x] 4 gates green.
