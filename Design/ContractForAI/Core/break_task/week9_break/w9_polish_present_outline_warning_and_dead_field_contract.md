...
- **S5** `src/modules/present/present-integration.test.ts` (MODIFY): thêm assert title-khớp-heading cho ≥2 template — với mỗi slide có `fromSectionId`, title **bắt đầu bằng số chương** (regex `^\d+(\.\d+)*\.\s`) **hoặc** so trực tiếp với heading đánh số kỳ vọng của template mẫu; khẳng định order liên tục + title không rỗng + không có chuỗi `[Cảnh báo:` lẫn trong `bullets` (chốt S2).

### Out of scope
- ❌ Thêm CSS panel `ws-present-*` nền tảng (→ contract `w9_fix_present_panel_styles_and_heading_numbering_contract.md`; S2 chỉ thêm style cho khối cảnh báo mới).
- ❌ Làm robust ghép số chương (→ contract fix S1).
- ❌ Đổi công thức phân bổ thời lượng / chia speaker (Groups B/C đúng).
- ❌ Đổi cap 5 bullets hay luật rút câu chủ đề (w9a — chủ ý).
- ❌ `buildSpeakers` fallback "1 speaker mặc định" khi thiếu members (w9c cho phép `[]` — không phải lỗi; để sau nếu muốn cải thiện UX).
- ❌ Dep mới / network / AI.

## 3. Checklist
- [ ] `SlideOutline` có field cảnh báo riêng (`brokenEvidenceNotes`/`warnings`) ở `present.ts` + zod + CanonicalTypes §9; `bullets` **không** còn chứa `"[Cảnh báo:"`.
- [ ] Cảnh báo render ở khối riêng (không sửa/xóa như bullet); không đếm vào cap 5.
- [ ] `estimatedSeconds`: **populate** hoặc **gỡ** dứt điểm (không để field chết); §9 đồng bộ.
- [ ] `generateSlideOutline`: parse mỗi section **một lần**; output bảo toàn.
- [ ] `present-integration.test.ts`: assert title khớp heading đánh số ≥2 template + không có cảnh báo lẫn trong bullets.
- [ ] ≤200 dòng/file; 4 gates xanh; lưu `build_output.txt` thật.
...
> ⛔ VibeCode Step 2: chưa chạm `src/` cho tới khi Approve. Đề xuất commit: (1) `refactor(present): move broken-evidence warning out of bullets into notes field`; (2) `refactor(present): drop dead estimatedSeconds (or populate from timeline)`; (3) `perf(present): parse each section markdown once`; (4) `test(present): assert numbered title parity across templates`; (5) `docs(w9): real build_output after present polish`.
