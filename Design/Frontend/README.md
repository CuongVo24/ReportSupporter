...
* `0.ArtDirection.md` — ✅ **implemented** + **§12 hệ ẩn dụ chữ ký "bàn viết → tờ nộp"** (định danh độc bản).
* `1.Foundations/` — token canonical đã có; `Typography.md` = ✅ **implemented**.
* `2.Components/` — ✅ **implemented & axe automated (W15):** Button · Input · Select · Textarea · Badge · Dialog · Tabs · Toast (khuôn `_ComponentSpecRule.md`).
* `3.Patterns/` — ✅ **implemented & axe automated (W15):** Empty · Error · Feedback · FormValidation · Loading có **case domain + bố cục**.
* `4.Layouts/` — ✅ **implemented & axe automated (W15):** AppShell (signature shell) · InformationArchitecture · Responsive.
* `5.Flows/` — ✅ **implemented & axe automated (W15):** Write · Export · Present theo khuôn **`_FlowSpecRule.md`** (screen spec: pane/primary/states/copy/gate/chữ-ký).
* `Other/` — ✅ **implemented & axe automated (W15):** VoiceAndContent (+ §8 nhịp deadline) · Icons · Motion · Accessibility (axe automated) · **`SignatureInteractions.md`** (5 hành vi độc bản).
